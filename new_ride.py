...
if 't' in form and 'rid' in form:
  ...
  if r[0] == 1:
    ...
    print "Location: bookings.py?k={}&cal={}".format(key, r[1])
    print ""
...
print '''<html>
<head>
<title>New booking for {} on {}</title>
...
if error_msg != "":
  print '<div id="error">{}</div>'.format(error_msg)


print '<div id="newbar">'
print '<a href="bookings.py?k={}">bookings</a>'.format(key)
print '</div>'
...
<div id="pick">
<div class="pstop">
<span class="st">{}</span>'''.format(pi[0])
if m:
...
<div id="drop">
<div class="dstop">
<span class="st">{}</span>'''.format(di[0])
if m:
  print '<img src="https://maps.googleapis.com/maps/api/staticmap?center={},{}&zoom=12&size=100x100&maptype=roadmap%20&markers=size:tiny%7C%7C{},{}&key={}"/>'.format(
...
for t in times:
  if t[3] != "0":
    fk["t"] = t[0]
    print '<div class="ride"><a href="{}?{}">{} => {}</a></div>'.format(
